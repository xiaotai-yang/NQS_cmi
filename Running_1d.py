...
parser.add_argument('--previous_training', dest = 'previous_training', action='store_true')
parser.add_argument('--dmrg', dest = 'dmrg', action='store_true')
parser.set_defaults(previous_training=False)
parser.set_defaults(dmrg=False)
...
parser.add_argument('--TQS_units', type = int, default=32)
...
if (model_type == "tensor_gru"):
    ...
    if previous_training == True:
        meanEnergy = jnp.load("result/meanE_1DRWKV" + "_Htype" + str(H_type) + "_L" + str(L) + "_patch" + str(p) + "_emb" + str(
        ...
        varEnergy = jnp.load("result/varE_1DRWKV" + "_Htype" + str(H_type) + "_L" + str(L) + "_patch" + str(p) + "_emb" + str(
            RWKV_emb) + "_layer" + str(RWKV_layer) + "_hidden" + str(RWKV_hidden) + "_ff" + str(
            RWKV_ff) + "_batch" + str(numsamples) + "_dmrg" + str(dmrg) + "_seed" + str(args.seed) + "_angle" + str(
            round(angle, 3)) + ".npy").tolist()
    ...
    if previous_training == True:
        meanEnergy = jnp.load("result/oneDTQS/meanE_1DTQS" + "_Htype" + str(H_type) + "_L" + str(L) + "_patch" + str(p) + "_layer" + str(
        ...
        varEnergy = jnp.load("result/oneDTQS/varE_1DTQS" + "_Htype" + str(H_type) + "_L" + str(L) + "_patch" + str(p) + "_layer" + str(
            TQS_layer) + "_ff" + str(TQS_ff) + "_units" + str(TQS_units) + "_head" + str(TQS_head) + "_batch" + str(
            numsamples) + "_dmrg" + str(dmrg) + "_seed" + str(args.seed) + "_angle" + str(round(angle, 3)) + ".npy").tolist()
        ...
        optimizer_state = optimizer.init(params)
    setting = dmrg
    fixed_params = L, p, TQS_layer, TQS_units, TQS_head
...
while(it<numsteps+eval_steps):
    ...
    if dmrg == True:
        sample_log_amp += batch_log_phase_dmrg(samples.reshape(-1, L*p), M0, M, Mlast)
    ...
    if it<numsteps:
        ...
        grads = lax.cond(gradient_clip, lambda x: jax.tree.map(clip_grad, x), lambda x: x, grads)
        ...
        if ((it+1)%1000 == 0):
            if (model_type == "tensor_gru"):
                ...
                with open(f"params/params_model1D{model_type}_Htype{H_type}_L{L}_patch{p}_layer{TQS_layer}_units{TQS_units}_head{TQS_head}_batch{numsamples}_dmrg{dmrg}_angle{round(angle, 3)}_seed{args.seed}.pkl", "wb") as f:
                    ...
                    pickle.dump(checkpoint, f)
                jnp.save(
                    "result/oneDTQS/meanE_1DTQS" + "_Htype" + str(H_type) + "_L" + str(L) + "_patch" + str(p) + "_layer" + str(
                        TQS_layer) + "_ff" + str(TQS_ff) + "_units" + str(TQS_units) + "_head" + str(
                        TQS_head) + "_batch" + str(numsamples) + "_dmrg" + str(dmrg) + "_seed" + str(
                        args.seed) + "_angle" + str(round(angle, 3)) + ".npy", jnp.array(meanEnergy).ravel())
                jnp.save(
                    "result/oneDTQS/varE_1DTQS" + "_Htype" + str(H_type) + "_L" + str(L) + "_patch" + str(p) + "_layer" + str(
                        TQS_layer) + "_ff" + str(TQS_ff) + "_units" + str(TQS_units) + "_head" + str(
                        TQS_head) + "_batch" + str(numsamples) + "_dmrg" + str(dmrg) + "_seed" + str(
                        args.seed) + "_angle" + str(round(angle, 3)) + ".npy", jnp.array(varEnergy).ravel())
...
if model_type == "tensor_gru":
    ...
    jnp.save("result/oneDTQS/evalmeanE_1DTQS"+"_Htype"+str(H_type)+"_L" + str(L) + "_patch" + str(p) + "_layer" + str(TQS_layer) + "_ff" + str(TQS_ff) + "_units" + str(TQS_units) + "_head" + str(TQS_head) + "_batch" + str(numsamples) +"_dmrg"+str(dmrg)+ "_seed" + str(args.seed)+"_angle"+str(round(angle, 3)) + ".npy", jnp.array(eval_meanEnergy))
    jnp.save("result/oneDTQS/evalvarE_1DTQS"+"_Htype"+str(H_type)+"_L" + str(L) + "_patch" + str(p) + "_layer" + str(TQS_layer) + "_ff" + str(TQS_ff) + "_units" + str(TQS_units) + "_head" + str(TQS_head) + "_batch" + str(numsamples)+"_dmrg"+str(dmrg) + "_seed" + str(args.seed) +"_angle"+str(round(angle, 3))+ ".npy", jnp.array(eval_varEnergy))
