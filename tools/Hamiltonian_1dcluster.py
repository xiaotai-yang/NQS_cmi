...
def local_element_indices_1d(num_body, pauli_array, loc_array, rotation=False, angle=0.0):
    ...
    if pauli_array.shape[-1] != num_body:
        raise ValueError(
            f"Array has incorrect body of interactions {pauli_array.shape[-1]}. Expected body of interactions is {num_body}.")
    ...
    for i in range(num_body + 1):  # z_number
        for j in range(num_body + 1 - i):  # x_number]
            ...
            ref_bulk_z_mask = jnp.array([True, False, True])
            ref_first_z_mask = jnp.array([False, True])
            ref_last_z_mask = jnp.array([True, False])
            x, y = jnp.cos(angle), jnp.sin(angle)
            if rotation:
                if (num_body == 3):  # extract the bulk part
                    # adding a minus sign when Z go to X
                    coe_arrays[i, j] = -(-1) ** (
                        jnp.sum((mask_z == False) & (mask_z != ref_bulk_z_mask), axis=1)) * x ** (
                                           jnp.sum(mask_z == ref_bulk_z_mask, axis=1)) * y ** (
                                           jnp.sum(mask_z != ref_bulk_z_mask, axis=1))
                elif num_body == 2:  # extract the first and the last part
                    coe_arrays_odd[i, j] = -(-1) ** (
                        jnp.sum((mask_z[::2] != ref_first_z_mask) & (mask_z[::2] == False), axis=1)) * x ** (
                                               jnp.sum(mask_z[::2] == ref_first_z_mask, axis=1)) * y ** (
                                               jnp.sum(mask_z[::2] != ref_first_z_mask,
                                                       axis=1))  # odd part represents the first ZX stablizer
                    coe_arrays_even[i, j] = -(-1) ** (
                        jnp.sum((mask_z[1::2] != ref_last_z_mask) & (mask_z[1::2] == False), axis=1)) * x ** (
                                                jnp.sum(mask_z[1::2] == ref_last_z_mask, axis=1)) * y ** (
                                                jnp.sum(mask_z[1::2] != ref_last_z_mask,
                                                        axis=1))  # even part represents the last XX stablizer
                    coe_arrays[i, j] = jnp.vstack((coe_arrays_odd[i, j], coe_arrays_even[i, j])).T.reshape(
                        -1)  # mix the odd part and the even part together
            if mask_x.sum() != 0:
...
def total_samples_1d(sample, xyloc_arrays):
    ...
    def single_samples_1d(sample, xy_loc_array_element):
        return sample.at[xy_loc_array_element[:, 0]].set(
            (sample[xy_loc_array_element[:, 0]] + 1) % 2)  # flip the corresponding site
...
def new_coe_1d(sample, coe_array_off_diag, yloc, zloc, rotation):
    ...
    for i in yloc:
        tmp_y.append(lax.cond(yloc[i].shape[1] != 0, lambda: vmap_ycoe(sample, yloc[i]),
                              lambda: jnp.repeat(jnp.array([1. + 0j]), yloc[i].shape[0])))
    for i in zloc:
        tmp_z.append(lax.cond(zloc[i].shape[1] != 0, lambda: vmap_zcoe(sample, zloc[i]),
                              lambda: jnp.repeat(jnp.array([1. + 0j]), zloc[i].shape[0])))
        if rotation:
...
@jax.jit
def diag_coe(samples, zloc_bulk_diag, zloc_fl_diag, coe_bulk_diag, coe_fl_diag):
    ...
    return coe_bulk + coe_fl


def loc_array_es(N):
    # Create a range of values from 0 to N-3
    i_values = jnp.arange(N - 2)[:, None]  # Reshape to use broadcasting
    ...
    return loc_array_bulk, loc_array_first_last


def off_diag_count(xy_loc_bulk, xy_loc_fl):
    off_diag_bulk_count, off_diag_fl_count = 0, 0
    ...
    return off_diag_bulk_count, off_diag_fl_count
...
def vmc_off_diag_es(N, p, angle, basis_rotation):
    ...
    if (basis_rotation == False):
        ...
        '''
        pauli_array_bulk, pauli_array_fl = jnp.repeat(jnp.array([3, 1, 3])[None], (N * p - 2),axis=0), jnp.array([[1, 3], [3, 1]])
        loc_array_bulk, loc_array_fl = loc_array_es(N * p)
    else:
        '''
        First repeat for each location then iterate over the combinations
        [[1,1,1]...,[1,1,1],[1,1,3], [1,1,3]..., [3,3,3],[3,3,3]]
        '''
        pauli_array_bulk, pauli_array_fl = (jnp.repeat(generate_combinations(3), (N * p - 2), axis=0),
                                                             jnp.repeat(generate_combinations(2), 2, axis=0))
        # The location that each Hamiltonian term acts on
        loc_array_bulk, loc_array_fl = loc_array_es(N * p)
        loc_array_bulk, loc_array_fl = jnp.tile(loc_array_bulk, (8, 1)), jnp.tile(loc_array_fl,(4, 1))
    ...
    if (basis_rotation == False):
        ...
        off_diag_bulk_count, off_diag_fl_count = off_diag_count(xy_loc_bulk, xy_loc_fl)
        off_diag_bulk_coe, off_diag_fl_coe = -jnp.ones(off_diag_bulk_count), -jnp.ones(off_diag_fl_count)
    else:
    ...
    zloc_bulk_diag, zloc_fl_diag= jnp.array([]), jnp.array([])
    coe_bulk_diag, coe_fl_diag = jnp.array([]), jnp.array([])
    ...
    if (3, 0) in xy_loc_bulk:
        if zloc_bulk[(3, 0)].size != 0:
            ...
            if (basis_rotation == False):
                ...
                # For xy_loc_bulk, the original term is ZXZ, for it rotate to ZZZ, it will obtain a cos(\theta)*sin^2(\theta) coeffiecient
                coe_bulk_diag = -jnp.ones(zloc_bulk_diag.shape[0]) * x **2 * y
        del xy_loc_bulk[(3, 0)]
        del yloc_bulk[(3, 0)]
        del zloc_bulk[(3, 0)]
    if (2, 0) in xy_loc_fl:
        if zloc_fl[(2, 0)].size != 0:
            ...
            if (basis_rotation == False):
                ...
                # XZ term rotate to ZZ term, it will obtain a cos(\theta)*sin(\theta) coeffiecient
                # ZX term rotate to ZZ term, it will obtain a sin^2(\theta) coeffiecient
                coe_fl_diag = jnp.concatenate((-jnp.ones(int(zloc_fl_diag.shape[0] / 2)) * x * y,
                                               -jnp.ones(int(zloc_fl_diag.shape[0] / 2)) * x * y ))
        del xy_loc_fl[(2, 0)]
        del yloc_fl[(2, 0)]
        del zloc_fl[(2, 0)]

    return (xy_loc_bulk, xy_loc_fl, yloc_bulk, yloc_fl, zloc_bulk, zloc_fl, off_diag_bulk_coe, off_diag_fl_coe, zloc_bulk_diag, zloc_fl_diag,
             coe_bulk_diag, coe_fl_diag)
